...
//       console.log("reminderdata",reminderData,editMode)
...
//       // Direct mapping when week starts on Sunday
//       return backendDay;
//     } else {
//       // When week starts on Monday:
//       // Backend: Monday=1, Tuesday=2, Wednesday=3, Thursday=4, Friday=5, Saturday=6, Sunday=0
//       // UI: Monday=0, Tuesday=1, Wednesday=2, Thursday=3, Friday=4, Saturday=5, Sunday=6
//       if (backendDay === 0) {
//         return 6; // Sunday
//       } else {
//         return backendDay - 1;
...
//     severity: "info",
//   });
//   // Flag to prevent useEffect from running on initial render
...
//     setToRecipients([]);
//     setCcRecipients([]);
//     setTargetEntity(null); // Reset target entity
//     setIsInitialMount(true); // Reset the initial mount flag
...
//             });
//           }
//         } else if (data.targetEntity.customEmails && Array.isArray(data.targetEntity.customEmails) && data.targetEntity.customEmails.length > 0) {
...
//       // Set the initial mount flag to false after populating the form
//       setIsInitialMount(false);
//     }
//   }, [reminderData, editMode, fieldOptions, defaultConfig.weekStartsOnSunday, weekdays, mapBackendDayToUiIndex]);
//   // Handlers
//   const handleWeeklyDayToggle = useCallback((day) => {
//     setWeeklyDays((prev) => {
//       const newDays = prev.includes(day)
//         ? prev.filter((d) => d !== day)
//         : [...prev, day].sort(
...
//   //   // Format target entity for backend
...
//   //     } else {
//   //       // If it's an object with attributeId
...
//   // Replace the handleAddReminder function with this corrected version:
...
//   const parseRRule = useCallback((rruleString) => {
//     try {
//       const parts = rruleString.split(";");
//       const rules = parts.reduce((acc, part) => {
//         const [key, val] = part.split("=");
...
//       }
//     }
//   }, [defaultConfig.hideError, DAYS, MONTHS, WEEK_OPTIONS, weekdays, dayAbbrevs]);
...
//   // Effect to handle RRule generation
//   useEffect(() => {
...
//   // Effect to parse RRule from props
//   useEffect(() => {
//     if (value && value !== generateRRule()) {
//       parseRRule(value);
//     }
//   }, [value, generateRRule, parseRRule]);
  
//   // Helper function to format recipients for display
//   const formatRecipientsForDisplay = useCallback((recipients) => {
//     if (!recipients || recipients.length === 0) return "None";
//     return recipients
//       .map((recipient) => {
//         if (recipient.customEmails && Array.isArray(recipient.customEmails)) {
...
//   // Helper function to format target entity for display
//   const formatTargetEntityForDisplay = useCallback((targetEntity) => {
//     if (!targetEntity) return "None";
//     if (targetEntity.customEmails && Array.isArray(targetEntity.customEmails) && targetEntity.customEmails.length > 0) {
...
//   // Helper function to format recurrence details for display
//   const formatRecurrenceDetails = useCallback((data) => {
//     if (!data.frequency) return "Not set";
//     let details =
//       data.frequency.charAt(0).toUpperCase() + data.frequency.slice(1);
//     if (data.interval && data.interval > 1) {
...
//               value={targetEntity}
//               onChange={(event, newValue) => {
//                 // Set the new value (single select)
...
const RRuleGenerator = ({
  ...
  const parseRRule = useCallback((rruleString) => {
    try {
      ...
      if (rules.FREQ) {
        const newRepeatType = rules.FREQ.charAt(0) + rules.FREQ.slice(1).toLowerCase();
      ...
      if (rules.FREQ === "YEARLY" && rules.BYMONTH) {
        ...
        const months = monthIndices
          .map((index) => MONTHS[index - 1]?.id)
          .filter((id) => id);
        if (months.length > 0 && JSON.stringify(months) !== JSON.stringify(yearlyMonths)) {
          setYearlyMonths(months);
        }
        if (months.length > 0 && JSON.stringify(months) !== JSON.stringify(yearlyWeekMonths)) {
      ...
      if (rules.FREQ === "YEARLY" && rules.BYMONTHDAY) {
        ...
        if (days.length > 0 && JSON.stringify(days) !== JSON.stringify(yearlyDays)) {
          setYearlyDays(days);
        }
      }
      if (rules.FREQ === "YEARLY" && rules.BYDAY) {
        ...
        if (weeks.length > 0 && JSON.stringify(weeks) !== JSON.stringify(yearlyWeeks)) {
          setYearlyWeeks(weeks);
        }
        if (weekDays.length > 0 && JSON.stringify(weekDays) !== JSON.stringify(yearlyWeekDays)) {
      ...
      if (rules.FREQ === "MONTHLY" && rules.BYMONTHDAY) {
        ...
        if (days.length > 0 && JSON.stringify(days) !== JSON.stringify(monthlyDays)) {
          setMonthlyDays(days);
        }
      }
      if (rules.FREQ === "MONTHLY" && rules.BYDAY) {
        ...
        if (weeks.length > 0 && JSON.stringify(weeks) !== JSON.stringify(monthlyWeeks)) {
          setMonthlyWeeks(weeks);
        }
        if (weekDays.length > 0 && JSON.stringify(weekDays) !== JSON.stringify(monthlyWeekDays)) {
      ...
      if (rules.FREQ === "WEEKLY" && rules.BYDAY) {
        const days = rules.BYDAY.split(",")
          ...
          .filter((id) => id);
        if (days.length > 0 && JSON.stringify(days) !== JSON.stringify(weeklyDays)) {
          setWeeklyDays(days);
        }
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Generate current RRule to compare with value
  ...
  // Effect to handle frequency type changes
  useEffect(() => {
    if (!isMounted.current || isInitialMount.current) return;
    
    // When frequency type changes, reset related fields
    if (previousRepeatType.current !== repeatType) {
      ...
      // Update the previous repeat type
      previousRepeatType.current = repeatType;
    }
  }, [repeatType, defaultConfig.yearly, defaultConfig.monthly, isMounted, isInitialMount]);
          ...
          {viewingReminderData ? (
            <div style={{ display: "flex", flexWrap: "wrap", gap: "16px" }}>
              {[
                ...
                {
                  label: "Target Entity",
                  value: formatTargetEntityForDisplay(viewingReminderData.targetEntity),
                },
                {
                  label: "Acknowledge Required",
                  value: viewingReminderData.acknowledgeRequired ? "Yes" : "No",
