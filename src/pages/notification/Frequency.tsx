...
//       return `Annually on ${month} ${day}`;
//     case "custom":
//       // For custom recurrence, we need to determine the actual frequency type
//       // and construct the appropriate UI string based on the recurrence parameters
//       if (monthOfYear !== undefined && dayOfYearMonth !== undefined) {
//         const monthNames = [
//           "January",
//           "February",
...
//         return `Annually on ${monthNames[monthOfYear - 1]} ${dayOfYearMonth}`;
//       }
//       // If it's a custom recurrence but we can't determine the specific type,
//       // check if there's an end date to indicate it's not a simple "once" occurrence
//       if (schedulerEndDate) {
//         return "Custom...";
//       }
//       return "Do not repeat";
    ...
    case "monthly":
      ...
      if (dayOfMonth && Array.isArray(dayOfMonth) && dayOfMonth.length > 0) {
        if (schedulerEndDate) {
          ...
          return `Monthly on day ${dayOfMonth[0]} until ${endDate}`;
        }
        return `Monthly on day ${dayOfMonth[0]}`;
      }
      // Check if it's a weekday occurrence
      if (
        ...
        if (schedulerEndDate) {
          const endDate = formatDateForDisplay(schedulerEndDate);
          return `Monthly on the ${occurrence} ${dayNames[dayOfWeekInMonth]} until ${endDate}`;
        }
        return `Monthly on the ${occurrence} ${dayNames[dayOfWeekInMonth]}`;
      }
      // Fallback
      if (schedulerEndDate) {
        const endDate = formatDateForDisplay(schedulerEndDate);
        return `Monthly on day ${selectedDate.getDate()} until ${endDate}`;
      }
      return `Monthly on day ${selectedDate.getDate()}`;
    case "yearly":
      // Check if it's a specific date
      if (monthOfYear !== undefined && dayOfYearMonth !== undefined) {
        ...
        if (schedulerEndDate) {
          const endDate = formatDateForDisplay(schedulerEndDate);
          return `Annually on ${monthNames[monthOfYear - 1]} ${dayOfYearMonth} until ${endDate}`;
        }
        return `Annually on ${monthNames[monthOfYear - 1]} ${dayOfYearMonth}`;
      }
      // Fallback - use the selected date to determine the yearly recurrence
      ...
      if (schedulerEndDate) {
        const endDate = formatDateForDisplay(schedulerEndDate);
        return `Annually on ${month} ${day} until ${endDate}`;
      }
      return `Annually on ${month} ${day}`;
    case "custom":
      // For custom recurrence, we need to determine the actual frequency type
      ...
      // check if there's an end date to indicate it's not a simple "once" occurrence
      if (schedulerEndDate) {
        return `Custom... until ${formatDateForDisplay(schedulerEndDate)}`;
      }
      return "Custom...";
    default:
      return frequency; // Fallback
...
export default function Frequency({ fieldOptions, notificationTypeId }) {
  ...
  if (row.frequency) {
    ...
    // If it's a custom recurrence with specific parameters, set those as well
    if (row.frequency === "custom" || (row.monthOfYear !== undefined && row.dayOfYearMonth !== undefined)) {
      // Set the custom recurrence parameters
      if (row.monthOfYear !== undefined && row.dayOfYearMonth !== undefined) {
      ...
      if (row.maxOccurrences && row.maxOccurrences > 0) {
        setEndsOption("after");
