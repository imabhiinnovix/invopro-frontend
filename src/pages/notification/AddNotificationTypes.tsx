...
//           borderRadius: 2,
//           p: 3,
//           width: isRoot ? "100%" : "auto", // Increased width for root
//           bgcolor: isRoot ? "background.paper" : "inherit", // Sleeker background
//           boxShadow: isRoot ? 3 : 1, // Subtle shadow for sleek look
...
//                 fontSize: "0.9rem", // Smaller font for sleek look
//               }}
//             >
...
//                 label="Logic"
//                 sx={{ fontSize: "0.85rem" }} // Smaller font size
//               >
...
//                         <MenuItem key={option._id} value={option._id}>
//                           <Checkbox
//                             checked={row.methods.includes(option._id)}
//                           />
//                           {option.medium}
//                         </MenuItem>
//                       ))}
...
//                       allFrequenciesSuccessful = false;
//                       toast.error(
//                         `Failed to create frequency for reminder ${reminder.id},
...
//       toast.error(
//         "Failed to create notification or frequencies. Please try again."
//       );
//     }
//   };
...
import Frequency from "./Fequency";
...
const TaskReminderInterface = ({ onChange }) => {
  ...
  const templateList = useGet<any>(["templateList"], `${GET.TEMPLATE_LIST}`, true);
  const mediumList = useGet<any>(["mediumList"], `${GET.MEDIUM_LIST}`, true);
  ...
  const addReminder = (setId) => {
    setReminderSets((prev) =>
      prev.map((set) => {
        ...
        if (!currentReminder.rrule) {
          toast.error("Please configure a valid recurrence rule");
          return set;
        }
  ...
  const formatReminderText = (reminder) => {
    // Basic formatting for RRule-based reminders
    let text = reminder.time;
    if (reminder.rrule) {
      text += ` (${reminder.rrule})`;
    } else {
      text += " (No recurrence rule set)";
    }
    return text;
  };

  useEffect(() => {
    // Transform reminderSets to match the expected output format
    const transformedData = reminderSets.map((set) => ({
      ...set,
      reminders: set.reminders.map((reminder) => ({
        ...reminder,
        frequency: reminder.rrule.includes("FREQ=DAILY")
          ? "daily"
          : reminder.rrule.includes("FREQ=WEEKLY")
          ? "weekly"
          : reminder.rrule.includes("FREQ=MONTHLY")
          ? "monthly"
          : "once",
        interval: parseInt(reminder.rrule.match(/INTERVAL=(\d+)/)?.[1] || "1"),
        daysOfWeek: reminder.rrule.match(/BYDAY=([^;]+)/)?.[1]?.split(",") || [],
        daysOfMonth:
          reminder.rrule.match(/BYMONTHDAY=([^;]+)/)?.[1]?.split(",").map(Number) || [],
        specificDate: reminder.rrule.includes("DTSTART")
          ? reminder.rrule.match(/DTSTART:(\d{8})/)?.[1]
          : undefined,
        repeatAnnually: reminder.rrule.includes("FREQ=YEARLY"),
      })),
      currentReminder: {
        ...set.currentReminder,
        frequency: set.currentReminder.rrule.includes("FREQ=DAILY")
          ? "daily"
          : set.currentReminder.rrule.includes("FREQ=WEEKLY")
          ? "weekly"
          : set.currentReminder.rrule.includes("FREQ=MONTHLY")
          ? "monthly"
          : "once",
        interval: parseInt(set.currentReminder.rrule.match(/INTERVAL=(\d+)/)?.[1] || "1"),
        daysOfWeek: set.currentReminder.rrule.match(/BYDAY=([^;]+)/)?.[1]?.split(",") || [],
        daysOfMonth:
          set.currentReminder.rrule.match(/BYMONTHDAY=([^;]+)/)?.[1]?.split(",").map(Number) || [],
        specificDate: set.currentReminder.rrule.includes("DTSTART")
          ? set.currentReminder.rrule.match(/DTSTART:(\d{8})/)?.[1]
          : undefined,
        repeatAnnually: set.currentReminder.rrule.includes("FREQ=YEARLY"),
      },
    }));
          ...
          >
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <RRuleGenerator
                  onChange={(rrule) =>
                    updateCurrentReminder(set.id, { rrule })
                  }
                  value={set.currentReminder.rrule}
                  config={{
          ...
          for (const row of set.dropdownRows) {
            if (row.template && row.methods.length > 0) {
              for (const medium of row.methods) {
                const frequencyPayload = {
                  notificationTypeId: notificationTypeId,
                  frequency: set.currentReminder.frequency,
                  interval: set.currentReminder.interval,
                  daysOfWeek: set.currentReminder.daysOfWeek,
                  daysOfMonth: set.currentReminder.daysOfMonth,
                  specificDate: set.currentReminder.specificDate,
                  repeatAnnually: set.currentReminder.repeatAnnually,
                  acknowledgeRequired: set.currentReminder.acknowledgement,
                  attachmentRequired: set.currentReminder.isAttached,
                  recipients: [],
                  medium: medium,
              ...
              if (row.template && row.methods.length > 0) {
                for (const medium of row.methods) {
                  const frequencyPayload = {
                    ...
                    repeatAnnually: reminder.repeatAnnually,
                    acknowledgeRequired: reminder.acknowledgement,
                    attachmentRequired: reminder.isAttached,
                    recipients: [],
                    medium: medium,
