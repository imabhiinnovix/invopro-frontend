...
//       console.log("operatorList response", operatorList.data);
//     }
//   }, [operatorList.data]);
...
//   // Update fieldOptions when entityId changes
//   useEffect(() => {
...
//     "submitted",
//   ];
//   const caseTypeOptions = ["PRI", "PoperatorRO", "TRA", "DES"];
...
//     // If operator changes and value is not required, clear the value
//     if (field === "operator") {
...
//       return null; // Hide value input if no field is selected or value is not required
//     }
...
//               updateRule(groupPath, index, "operator", ""); // Reset operator when field changes
//               updateRule(groupPath, index, "value", ""); // Reset value when field changes
//             }}
//             label="Select field..."
//           >
...
const ConditionRuleBuilder = ({ onChange, notificationTypeList }) => {
    ...
    if (notification.entityId && notificationTypeList.data?.data) {
      ...
      if (selectedEntity && selectedEntity.attributes) {
        const newFieldOptions = selectedEntity.attributes.map((attr) => ({
          ...
          label: attr.name,
          type: attr.type,
          attributeId: attr._id, // Store attributeId for mapping
        }));
        setFieldOptions(newFieldOptions);
      } else {
        setFieldOptions([]);
  ...
  React.useEffect(() => {
    // Transform the notification data to match backend structure
    const transformNotificationData = () => {
      ...
      return {
        ...
        organizationId: "64b7f92fcf1a3b001f2a1234", // Hardcoded for now, replace with actual value
        entityId: notification.entityId,
        triggerFieldId: "", // Add logic to set triggerFieldId if needed
        isActive: true,
        conditionGroups,
      };
    };
  ...
  }, [notification, fieldOptions, onChange]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await createNotification.mutateAsync({
        url: "/notivix/notification-setting/type/create",
        payload: notificationData,
      });
      console.log("Notification created successfully:", createNotification.data);
