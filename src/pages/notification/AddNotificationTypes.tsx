...
const ConditionRuleBuilder = ({
  ...
  const addRule = (groupPath) => {
    const newRule = {
      ...
      operator: "",
      value: "",
      timeUnit: "", // Added for date fields
    };
  ...
  const updateRule = (groupPath, index, field, value) => {
    ...
    if (field === "operator") {
      ...
      if (selectedOperator && !selectedOperator.valueRequired) {
        ...
        group.rules[index].timeUnit = ""; // Reset timeUnit for non-value-required operators
      }
    }
    setNotification(updatedNotification);
  };
          ...
          if (rule.logic) {
            ...
            if (fieldOption) {
              conditions.push({
                ...
                value: rule.value,
                timeUnit: rule.timeUnit || "", // Include timeUnit for date fields
              });
            }
          }
        });
