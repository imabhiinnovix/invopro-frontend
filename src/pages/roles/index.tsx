...
//     severity: "error",
//   });
//   const [formattedRoleDetails, setFormattedRoleDetails] = useState<
//     RoleDetail[]
//   >([]);

//   const initialPermissions = Object.keys(permissions ?? {}).reduce(
//     (acc, resourceType) => {
//       acc[resourceType] = Object.keys(permissions[resourceType] ?? {}).reduce(
//         (permAcc, permKey) => {
...
//     const permissionIds =
//       Array.isArray(formattedRoleDetails) && formattedRoleDetails.length > 0
...
export default function Roles() {
  ...
  // Log raw role detail response and format/save role details
  useEffect(() => {
    if (roleDetail?.data) {
      console.log("Raw Role Detail Response:", roleDetail.data);
      if (roleDetail.data.success && roleDetail.data.data) {
        const formattedData = roleDetail.data.data.map((detail) => ({
          ...detail,
          permissionId: {
  ...
  const handleEdit = (row: Role) => {
    ...
    // Reset formattedRoleDetails to ensure fresh data
    setFormattedRoleDetails([]);
