...
export default function NotivixDataSourceSummary() {
...
const [selectedSegregationField, setSegregationField] = useState<string | null>(null);
...
console.log('uniqueMonths',uniqueMonths);
  ...
  const handleFilter = async (filters: any, year: any, month: any, segregation: any) => {
  ...
  setSegregationField(segregation || null);
  ...
  const sourceVersionData = useGet<ApiResponse>(
    ...
    `${GET.SOURCE_VERSION_DATA}?dataSourceId=${encodeURIComponent(
      ...
      debouncedSearchValue || ""
    )}&isSummary=true&segregationField=${selectedSegregationField || ""}`,
    !!valueId
  );
 ...
 const handleOpenInNewTab = (rowData?: Record<string, any>) => {
  ...
  const columns = useMemo(() => {
    ...
    const displayFields =
      listCurrentData.fieldSettings.filter(
        (field) => (field.isSummaryDisplayEnable || field.mappedAttributeName === selectedSegregationField) && field.mappedAttributeName
      ) || [];
  ...
  const segregationFields = listCurrentData?.fieldSettings?.filter(
  (field) => field.isSummarySegregation && field.mappedAttributeName
);
  ...
  useEffect(() => {
    ...
    const formattedRows = rawData.map((item, index) => {
  const row: Record<string, any> = {
    _id: `summary-${index}`, // ✅ always generate ID
  };
    ...
    : "Data Source Summary"
      ...
      {valueId && (
        <NotivixFiltersModal
          ...
          segregationFields={segregationFields}
          selectedSegregationField={selectedSegregationField}
          isSummary={true}
        />
      )}
