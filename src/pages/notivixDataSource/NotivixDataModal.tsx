...
export const NotivixDataModal: React.FC<ModelSectionProps> = ({
  ...
  const handleSaveClick = async () => {
    try {
      ...
      if (modalMode === 'add' || modalMode === 'edit') {
        ...
        handleSave(payload);
        refreshData();
        const successMessage = modalMode === 'add' ? 'Record added successfully!' : 'Record updated successfully!';
        toast.success(successMessage);
      } else if (modalMode === 'filter') {
        handleSave(formData);
      }
  ...
  function normalizeMultiOptionValue(raw: any, options: { id: string; label: string }[]) {
    let values: string[] = [];
    if (Array.isArray(raw)) {
      values = raw;
    } else if (typeof raw === 'string') {
      ...
      case 'multioption': {
        ...
        return (
          <Autocomplete
            ...
            options={options}
            value={selectedOptions}
            onChange={(e, val) => handleFieldChange(val.map((item) => (typeof item === 'string' ? item : item.id)))}
            disabled={!isFieldEditable}
            renderTags={(value, getTagProps) =>
              value.map((option, index) => (
                <Chip
            ...
            renderInput={(params) => (
              <TextField
                ...
                error={!!fieldErrors[fieldName]}
                helperText={fieldErrors[fieldName] || ''}
                placeholder={!isFieldEditable ? '' : isReferenceMulti ? 'Select option' : 'Type or select'}
              />
            )}
          />
        );
      ...
      case 'number':
        return (
          <TextField
            ...
            value={value || ''}
            onChange={(e) =>
              handleFieldChange(e.target.value === '' || isValidNumber(e.target.value) ? e.target.value : value)
            }
            variant="outlined"
            fullWidth
            disabled={!isFieldEditable}
