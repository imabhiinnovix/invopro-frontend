...
interface NotivixFiltersModalProps {
  ...
  month?: string, updatedDateRange?: any, segregationField?: any,) => void;
...
const InvoiceFiltersModal: React.FC<NotivixFiltersModalProps> = ({
  open,
  onClose,
  onApplyFilters,
  currentFilters = {},
  ...
  const [updatedRange, setUpdatedRange] = useState<DateObject[]>([]);
...
useEffect(() => {
  ...
  const handleApplyFilters = () => {
    ...
    // ✅ Updated Date separately
  const updatedDateRange = {
    startDate: updatedRange?.[0]?.format?.("YYYY-MM-DD") || "",
    endDate: updatedRange?.[1]?.format?.("YYYY-MM-DD") || "",
  };
    onApplyFilters(transformedFilters, finalYear, finalMonth,updatedDateRange, segregationField);
    onClose();
  };
...
setUpdatedRange([]);
  setSegregationField(null);
    ...
    onApplyFilters({ ...defaultFilters }, undefined, undefined, undefined, undefined);
    onClose();
  };
...
{isShowYearMonth && ( <Box sx={{ gridColumn: "1 / -1" }}>
  <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
    <Box sx={{ flex: 1 }}>
     <DatePicker
  value={updatedRange}
