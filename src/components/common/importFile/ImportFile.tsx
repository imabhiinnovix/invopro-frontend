...
// const startGlobalPolling = (pollingId: string, callback: () => void) => {
...
// // Stop global polling
// const stopGlobalPolling = (pollingId: string) => {
//   if (globalPollingState.activePolls.has(pollingId)) {
//     clearTimeout(globalPollingState.activePolls.get(pollingId));
...
//   // Check for existing polling on mount
//   useEffect(() => {
//     isMountedRef.current = true;

//     // Check if there's an active polling session
//     const activePollingId = sessionStorage.getItem('activePollingId');
//     if (activePollingId && globalPollingState.isPolling) {
//       setIsPolling(true);
//       setPollingId(activePollingId);
//       pollingRef.current = {
//         isPolling: true,
//         pollingId: activePollingId,
//       };
//     }
...
//   // Sync polling state with global state
//   useEffect(() => {
//     pollingRef.current = {
//       isPolling,
//       pollingId,
...
//   // Custom polling hook that works independently of component lifecycle
//   const usePersistentPolling = () => {
//     const [pollingTimestamp, setPollingTimestamp] = useState(Date.now());
...
//     return { pollingTimestamp, triggerPoll };
//   };

//   const { pollingTimestamp, triggerPoll } = usePersistentPolling();
...
//             // Start global polling
//             startGlobalPolling(id, triggerPoll);
//           } else if (data?.status === "completed") {
//             toast.success("File processed successfully!");
//             handleCancel();
//           } else if (data?.status === "failed") {
...
//   // Robust polling effect that continues even after navigation
//   useEffect(() => {
//     // Only run if we have a polling ID and polling is active
...
//             navigate(`/notivix/validation-errors/${pollingId}`);
//           } else {
//             // Use window.location if component is not mounted
//             window.location.href = `/notivix/validation-errors/${pollingId}`;
//           }
//         } else {
//           // Still processing, schedule next poll
//           startGlobalPolling(pollingId, triggerPoll);
//         }
//       } else if (pollingData.isError) {
...
//     // No cleanup function - we want polling to continue independently
//   }, [pollingData, pollingId, isPolling, navigate, triggerPoll]);

//   // Cleanup global polling when component unmounts
//   useEffect(() => {
//     return () => {
//       // Only stop polling if this component started it
//       if (pollingRef.current.pollingId && pollingRef.current.isPolling) {
//         stopGlobalPolling(pollingRef.current.pollingId);
//       }
//     };
//   }, []);
...
export const startGlobalPolling = (pollingId: string, callback: () => void) => {
  ...
  const timer = setTimeout(() => {
    console.log("Executing polling callback for:", pollingId);
    callback();
...
export const stopGlobalPolling = (pollingId: string) => {
  console.log("Stopping global polling for:", pollingId);
...
if (typeof window !== "undefined") {
  ...
  if (activePollingId && pollingStartTime) {
    ...
    if (elapsed < 5 * 60 * 1000) {
      globalPollingState.isPolling = true;
      globalPollingState.pollingId = activePollingId;
      console.log("Resuming polling for:", activePollingId);
    } else {
...
const GlobalPollingManager: React.FC = () => {
  ...
  useEffect(() => {
    if (globalPollingState.isPolling && pollingId) {
      console.log("Polling effect triggered for:", pollingId);
  ...
  useEffect(() => {
    ...
    if (globalPollingState.isPolling && globalPollingState.pollingId) {
      console.log(
        "Found active polling session for:",
        globalPollingState.pollingId
      );
    ...
    return () => {
      // console.log("GlobalPollingManager unmounting");
      isMountedRef.current = false;
    };
  }, []);
  ...
  useEffect(() => {
    ...
    }, 2000); // Check every 2 seconds to ensure we don't miss the 5-second window
      ...
      >
        <DialogContent>
          ...
          >
            ...
            <Typography variant="body1">
              Your files have been uploaded and are now in the processing queue.
              You may close this message and continue using the platform. Once
              the processing is complete, you will be notified!
            </Typography>
          </Box>
        </DialogContent>
        <DialogActions
          sx={{ justifyContent: "center", pb: STYLE_GUIDE.SPACING.s3 }}
        >
          <Button
            variant="contained"
            onClick={() => setShowProcessingDialog(false)}
