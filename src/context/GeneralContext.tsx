...
export interface GeneralContextType {
  ...
  setReadingPanePlace: Dispatch<SetStateAction<'left' | 'right' | 'bottom' | 'top'>>;
}
...
const GeneralProvider = ({ children }: GeneralProviderProps) => {
  ...
  const [adminSidebarWidth, setAdminSidebarWidth] = useState<'3%' | '20%'>('20%');
  const [readingPanePlace, setReadingPanePlace] = useState<'left' | 'right' | 'bottom' | 'top'>('top');
